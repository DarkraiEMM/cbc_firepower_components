...
public class LargeAutocannonAmmoBoxBlockEntity extends AutocannonAmmoContainerBlockEntity {
	...
	private LazyOptional<IItemHandler> largeInventory = LazyOptional.of(() -> new LargeAutocannonAmmoBoxItemHandler(this));
	...
	public <T> LazyOptional<T> getCapability(Capability<T> cap, @Nullable Direction side) {
		if (cap == ForgeCapabilities.ITEM_HANDLER)
			return this.largeInventory.cast();
	...
	public void invalidateCaps() {
		...
		this.largeInventory.invalidate();
	}

	@Override
	public void reviveCaps() {
		super.reviveCaps();
		this.largeInventory = LazyOptional.of(() -> new LargeAutocannonAmmoBoxItemHandler(this));
	}
	...
	void insertStoredStack(int slot, ItemStack stack, int amount) {
		...
		this.setStoredStack(slot, copyWithCount(stack, newCount));
		this.setChanged();
	}
	...
	ItemStack extractStoredStack(int slot, int amount, boolean simulate) {
		...
		ItemStack extracted = copyWithCount(stored, extractedCount);
		if (!simulate) {
			this.setStoredStack(slot, copyWithCount(stored, storedCount - extractedCount));
			this.setChanged();
		}
		return extracted;
	}
	...
	private void setStoredStack(int slot, ItemStack stack) {
		...
		if (count > 0)
			safeStack = copyWithCount(stack, Math.min(count, MAX_SERIALIZED_STACK_COUNT));
		if (slot == 1) {
	...
	private void syncSerializedStacks() {
		this.setStoredStack(0, this.mainAmmoCount <= 0 ? ItemStack.EMPTY : copyWithCount(super.getMainAmmoStack(), this.mainAmmoCount));
		this.setStoredStack(1, this.tracerAmmoCount <= 0 ? ItemStack.EMPTY : copyWithCount(super.getTracerStack(), this.tracerAmmoCount));
	}
