...
/** Supplies Radar's API yaw handler with the same shaft input used by its CBC handler. */
@Pseudo
@Mixin(targets = "com.happysg.radar.block.controller.yaw.ApiMountYaw", remap = false)
public abstract class ApiMountYawSpeedMixin {
	@Redirect(method = "tick", at = @At(value = "INVOKE", target =
		"Lcom/happysg/radar/block/controller/yaw/AutoYawControllerBlockEntity;getSpeed()F"), require = 1)
