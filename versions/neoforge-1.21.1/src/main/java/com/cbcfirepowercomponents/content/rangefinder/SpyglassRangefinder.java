...
public final class SpyglassRangefinder {
	...
	public static void measure(ServerPlayer player) {
		...
		OptionalDouble distance = SableDistanceCompat.distance(
			player.level(), player.getEyePosition(), hit.getLocation());
		if (distance.isEmpty()) {
			player.displayClientMessage(Component.translatable(
				"item.cbc_firepower_components.spyglass_rangefinder.unavailable"), true);
			return;
		}

		player.displayClientMessage(Component.translatable(
			"item.cbc_firepower_components.spyglass_rangefinder.distance",
			String.format(Locale.ROOT, "%.1f", distance.getAsDouble())), true);
	}
}
